    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if conn:
                ...
                for answer in page_three_question_one_answers:
                    query_one = "SELECT id FROM answers WHERE questionsId = 21 AND answer = %s"
                    cursor.execute(query_one, (answer,))
                    answer_one = cursor.fetchone()
                    if answer_one:
                        answer_one_id = answer_one[0]
                        print(f"Page three, answer one ID: {answer_one_id}")
                        cursor.execute(sql, (answer_one_id, user_id, question_one_id))
                    else:
                        print("No answer found for Page three, question one")
                        return redirect(url_for('page_three.page_three_technology_adoption'))
                    
                for answer in page_three_question_two_answers:
                    query_two = "SELECT id FROM answers WHERE questionsId = 22 AND answer = %s"
                    cursor.execute(query_two, (answer,))
                    answer_two = cursor.fetchone()
                    if answer_two:
                        answer_two_id = answer_two[0]
                        print(f"Page three, answer two ID: {answer_two_id}")
                        cursor.execute(sql, (answer_two_id, user_id, question_two_id))
                    else:
                        print("No answer found for Page three, question two")
                        return redirect(url_for('page_three.page_three_technology_adoption'))
                    
                for answer in page_three_question_three_answers:
                    query_three = "SELECT id FROM answers WHERE questionsId = 23 AND answer = %s"
                    cursor.execute(query_three, (answer,))
                    answer_three = cursor.fetchone()
                    if answer_three:
                        answer_three_id = answer_three[0]
                        print(f"Page three, answer two ID: {answer_three_id}")
                        cursor.execute(sql, (answer_three_id, user_id, question_three_id))
                    else:
                        print("No answer found for page three, question two")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_four_answers:
                    query_four = "SELECT id FROM answers WHERE questionsId = 24 AND answer = %s"
                    cursor.execute(query_four, (answer,))
                    answer_four = cursor.fetchone()
                    if answer_four:
                        answer_four_id = answer_four[0]
                        print(f"Page three, answer four ID: {answer_four_id}")
                        cursor.execute(sql, (answer_four_id, user_id, question_four_id))
                    else:
                        print("No answer found for Page three, question four")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_five_answers:
                    query_five = "SELECT id FROM answers WHERE questionsId = 25 AND answer = %s"
                    cursor.execute(query_five, (answer,))
                    answer_five = cursor.fetchone()
                    if answer_five:
                        answer_five_id = answer_five[0]
                        print(f"Page three, answer five ID: {answer_five_id}")
                        cursor.execute(sql, (answer_five_id, user_id, question_five_id))
                    else:
                        print("No answer found for Page three, question five")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_six_answers:
                    query_six = "SELECT id FROM answers WHERE questionsId = 26 AND answer = %s"
                    cursor.execute(query_six, (answer,))
                    answer_six = cursor.fetchone()
                    if answer_six:
                        answer_six_id = answer_six[0]
                        print(f"Page three, answer six ID: {answer_six_id}")
                        cursor.execute(sql, (answer_six_id, user_id, question_six_id))
                    else:
                        print("No answer found for Page three, question six")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_seven_answers:
                    query_seven = "SELECT id FROM answers WHERE questionsId = 27 AND answer = %s"
                    cursor.execute(query_seven, (answer,))
                    answer_seven = cursor.fetchone()
                    if answer_seven:
                        answer_seven_id = answer_seven[0]
                        print(f"Page three, answer seven ID: {answer_seven_id}")
                        cursor.execute(sql, (answer_seven_id, user_id, question_seven_id))
                    else:
                        print("No answer found for Page three, question seven")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_eight_answers:
                    query_eight = "SELECT id FROM answers WHERE questionsId = 28 AND answer = %s"
                    cursor.execute(query_eight, (answer,))
                    answer_eight = cursor.fetchone()
                    if answer_eight:
                        answer_eight_id = answer_eight[0]
                        print(f"Page three, answer eight ID: {answer_eight_id}")
                        cursor.execute(sql, (answer_eight_id, user_id, question_eight_id))
                    else:
                        print("No answer found for Page three, question eight")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_nine_answers:
                    query_nine = "SELECT id FROM answers WHERE questionsId = 29 AND answer = %s"
                    cursor.execute(query_nine, (answer,))
                    answer_nine = cursor.fetchone()
                    if answer_nine:
                        answer_nine_id = answer_nine[0]
                        print(f"Page three, answer nine ID: {answer_nine_id}")
                        cursor.execute(sql, (answer_nine_id, user_id, question_nine_id))
                    else:
                        print("No answer found for Page three, question nine")
                        return redirect(url_for('page_three.page_three_technology_adoption'))

                for answer in page_three_question_ten_answers:
                    query_ten = "SELECT id FROM answers WHERE questionsId = 30 AND answer = %s"
                    cursor.execute(query_ten, (answer,))
                    answer_ten = cursor.fetchone()
                    if answer_ten:
                        answer_ten_id = answer_ten[0]
                        print(f"Page three, answer ten ID: {answer_ten_id}")
                        cursor.execute(sql, (answer_ten_id, user_id, question_ten_id))
                    else:
                        print("No answer found for Page three, question ten")
                        return redirect(url_for('page_three.page_three_technology_adoption'))
